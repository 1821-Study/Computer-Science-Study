...
> 플러시(`flush()`)는 영속성 컨텍스트의 변경 내용르 데이터베이스에 반영한다.

### 플러시의 실행

1. 변경 감지(`dirty checking`)가 동작하여 영속성 컨텍스트에 있는 모든 엔티티를 스냅샷과 비교해서 수정된 엔티티를 찾는다.
...
* `em.persist()`를 호출하여 `memberA`, `memberB`, `memberC`를 영속 상태로 만들었다.
* 해당 엔팉티들은 영속성 컨텍스트에 존재하지만 데이터베이스에 반영되지 않은 상태이다.
* JPQL은 SQL로 바로 변환되어 데이터베이스에서 엔티티를 조회한다.
  * 이때 데이터베이스에는 `memberA`, `memberB`, `memberC`는 아직 데이터베이스에 없으므로 쿼리 되지 않는다.
  * 이러한 상황을 방지하지 위해서 JPQL이 실행될 때 플러시를 자동으로 호출한다.
